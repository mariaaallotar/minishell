...
/*   Updated: 2024/09/26 11:54:42 by eberkowi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void add_out_or_append(t_main *main, t_tokens **tokens, int cmd_id, int *spl_id)
{
	...
	if (!ft_strncmp(main->split_input[*spl_id], ">", 2))
	{
		(*tokens)[cmd_id].redirect_out = &main->split_input[*spl_id + 1];
		(*spl_id) += 2;
	}
	else if (!ft_strncmp(main->split_input[*spl_id], ">>", 3))
	{
		(*tokens)[cmd_id].redirect_append = &main->split_input[*spl_id + 1];
		(*spl_id) += 2;
	}
}
